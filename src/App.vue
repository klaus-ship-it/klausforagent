...
type RelationshipMetric = 'firstDepositCount' | 'firstDepositAmount' | 'registeredCount'
const relationshipSortMode = ref<Record<string, string>>({})
const relationshipFilterEnabled = ref<Record<string, boolean>>({})
...
function relationshipDetailRows(type: 'agents' | 'players', scope: 'all' | 'direct') {
  let rows = [...relationshipRows(type, scope)]
  const firstDepositFilter = relationshipFilterEnabled.value[relationshipMetricKey(type, scope, 'firstDepositCount')] || relationshipFilterEnabled.value[relationshipMetricKey(type, scope, 'firstDepositAmount')]
  const registeredFilter = relationshipFilterEnabled.value[relationshipMetricKey(type, scope, 'registeredCount')]
  if (firstDepositFilter) rows = rows.filter((row) => relationshipInCycle(relationshipFirstDeposit(row)?.at))
  if (registeredFilter) rows = rows.filter((row) => relationshipInCycle(relationshipRegisterAt(row)))
  const sort = relationshipSortMode.value[relationshipMetricKey(type, scope, 'firstDepositCount')] || relationshipSortMode.value[relationshipMetricKey(type, scope, 'firstDepositAmount')] || relationshipSortMode.value[relationshipMetricKey(type, scope, 'registeredCount')]
...
function toggleRelationshipFilter(key: string) {
  relationshipFilterEnabled.value[key] = !relationshipFilterEnabled.value[key]
}
    ...
    <div>
            ...
            <section v-else-if="activeKey === 'players'">
              ...
              <template v-else-if="selectedPlayer">
                ...
                <div v-else-if="playerDetailTab === 'asset'" class="player-report-panel detail-page-card"><div class="report-summary-grid"><div><span>異動總筆數</span><strong>18 筆</strong></div><div><span>本月儲值</span><strong>{{ selectedPlayer.currency }} {{ selectedPlayer.deposit.toLocaleString() }}</strong></div><div><span>本月提領</span><strong>{{ selectedPlayer.currency }} 4,200</strong></div></div><div class="player-report-table"><div class="player-report-head"><span>發生時間</span><span>帳變類型</span><span>帳變金額</span><span>餘額</span><span>狀態／明細</span></div><div v-for="record in playerAssetRecords" :key="record.time" class="player-report-row"><time>{{ record.time }}</time><span>{{ record.type }}</span><strong :class="record.amount > 0 ? 'positive' : 'negative'">{{ record.amount > 0 ? '+' : '' }}{{ selectedPlayer.currency }} {{ Math.abs(record.amount).toLocaleString() }}</strong><strong>{{ selectedPlayer.currency }} {{ record.balance.toLocaleString() }}</strong><span><NTag size="small" round :type="record.status === '成功' ? 'success' : 'warning'">{{ record.status }}</NTag><small class="mode-summary">{{ record.detail }}</small></span></div></div></div>
                 ...
                 <div v-else-if="playerDetailTab === 'invite'" class="detail-list detail-page-card"><div class="detail-list-row"><span>註冊使用推廣碼</span><strong class="code-text">{{ selectedPlayer.referralCode || '未記錄' }}</strong></div><div class="detail-list-row"><span>自身邀請碼</span><strong class="code-text">{{ selectedPlayer.inviteCode }}</strong></div><div class="detail-list-row"><span>邀請總人數</span><strong>3 人</strong></div><div class="detail-list-row"><span>有效邀請</span><strong>2 人</strong></div><div class="detail-list-row"><span>待生效邀請</span><strong>1 人</strong></div></div>
                 <div v-else class="agent-detail-grid detail-page-card"><div class="full"><span>完整樹狀路徑</span><strong>{{ displayAgentPath(selectedPlayer.path) }}</strong></div><div><span>玩家層級</span><strong>{{ selectedPlayer.agentLevel }}</strong></div><div><span>所屬幣別</span><strong>{{ selectedPlayer.currency }}</strong></div><div><span>直屬代理</span><strong>north_team</strong></div><div><span>歸屬方式</span><strong>代理推廣碼</strong></div><div><span>資料可見範圍</span><strong>所屬代理線</strong></div><div class="full"><span>代理後台權限</span><p class="modal-help">總代理與一般代理可以查看所屬下級；轉線、狀態管理與編輯資料僅由運營商執行。</p></div></div></div></div>
              </template>
            </section>
          ...
          <div v-if="showAgentDetail && detailTab === 'relationship' && selectedAgent" class="relationship-clean-panel relationship-clean-overlay">
            ...
            <div class="relationship-clean-rows">
              <div class="relationship-clean-row">
                <div class="relationship-clean-trigger" role="button" tabindex="0" @click="toggleRelationshipSection('totalAgents')"><div class="relationship-clean-title-row"><span class="relationship-clean-title">總下級（含直屬）代理</span><em>{{ relationshipExpanded.totalAgents ? '收闔' : '展開' }}⌄</em></div><div class="relationship-clean-metrics"><div class="relationship-clean-metric"><small>代理人數</small><strong>{{ relationshipRows('agents', 'all').length }} 位</strong></div><div class="relationship-clean-metric"><small>總儲值金額</small><strong>{{ selectedAgent.currency }} {{ relationshipSum('agents', 'all', 'deposit').toLocaleString() }}</strong></div><div class="relationship-clean-metric"><small>總有效投注</small><strong>{{ selectedAgent.currency }} {{ relationshipSum('agents', 'all', 'bet').toLocaleString() }}</strong></div><div class="relationship-clean-metric relationship-cycle-metric"><div class="relationship-metric-label"><small>結算週期首存人數</small><div class="relationship-metric-tools"><select class="relationship-metric-sort" :value="relationshipSortMode[relationshipMetricKey('agents', 'all', 'firstDepositCount')] ?? ''" @click.stop @change="setRelationshipSort(relationshipMetricKey('agents', 'all', 'firstDepositCount'), $event)"><option value="">排序</option><option value="firstDepositDesc">時間由近而遠</option></select><button type="button" class="relationship-metric-filter" :class="{ active: relationshipFilterEnabled[relationshipMetricKey('agents', 'all', 'firstDepositCount')] }" @click.stop="toggleRelationshipFilter(relationshipMetricKey('agents', 'all', 'firstDepositCount'))">{{ relationshipFilterEnabled[relationshipMetricKey('agents', 'all', 'firstDepositCount')] ? '已篩選' : '篩選' }}</button></div></div><strong>{{ relationshipCycleCount('agents', 'all') }} 位</strong></div><div class="relationship-clean-metric relationship-cycle-metric"><div class="relationship-metric-label"><small>結算週期首存總金額</small><div class="relationship-metric-tools"><select class="relationship-metric-sort" :value="relationshipSortMode[relationshipMetricKey('agents', 'all', 'firstDepositAmount')] ?? ''" @click.stop @change="setRelationshipSort(relationshipMetricKey('agents', 'all', 'firstDepositAmount'), $event)"><option value="">排序</option><option value="firstDepositAmountDesc">金額由大到小</option></select><button type="button" class="relationship-metric-filter" :class="{ active: relationshipFilterEnabled[relationshipMetricKey('agents', 'all', 'firstDepositAmount')] }" @click.stop="toggleRelationshipFilter(relationshipMetricKey('agents', 'all', 'firstDepositAmount'))">{{ relationshipFilterEnabled[relationshipMetricKey('agents', 'all', 'firstDepositAmount')] ? '已篩選' : '篩選' }}</button></div></div><strong>{{ relationshipCycleAmount('agents', 'all') }}</strong></div><div class="relationship-clean-metric relationship-cycle-metric"><div class="relationship-metric-label"><small>結算週期註冊人數</small><div class="relationship-metric-tools"><select class="relationship-metric-sort" :value="relationshipSortMode[relationshipMetricKey('agents', 'all', 'registeredCount')] ?? ''" @click.stop @change="setRelationshipSort(relationshipMetricKey('agents', 'all', 'registeredCount'), $event)"><option value="">排序</option><option value="registeredDesc">時間由近而遠</option></select><button type="button" class="relationship-metric-filter" :class="{ active: relationshipFilterEnabled[relationshipMetricKey('agents', 'all', 'registeredCount')] }" @click.stop="toggleRelationshipFilter(relationshipMetricKey('agents', 'all', 'registeredCount'))">{{ relationshipFilterEnabled[relationshipMetricKey('agents', 'all', 'registeredCount')] ? '已篩選' : '篩選' }}</button></div></div><strong>{{ relationshipCycleRegistered('agents', 'all') }} 位</strong></div></div></div>
              ...
              <div class="relationship-clean-row">
                <div class="relationship-clean-trigger" role="button" tabindex="0" @click="toggleRelationshipSection('directAgents')"><div class="relationship-clean-title-row"><span class="relationship-clean-title">直屬下級代理</span><em>{{ relationshipExpanded.directAgents ? '收闔' : '展開' }}⌄</em></div><div class="relationship-clean-metrics"><div class="relationship-clean-metric"><small>代理人數</small><strong>{{ relationshipRows('agents', 'direct').length }} 位</strong></div><div class="relationship-clean-metric"><small>總儲值金額</small><strong>{{ selectedAgent.currency }} {{ relationshipSum('agents', 'direct', 'deposit').toLocaleString() }}</strong></div><div class="relationship-clean-metric"><small>總有效投注</small><strong>{{ selectedAgent.currency }} {{ relationshipSum('agents', 'direct', 'bet').toLocaleString() }}</strong></div><div class="relationship-clean-metric relationship-cycle-metric"><div class="relationship-metric-label"><small>結算週期首存人數</small><div class="relationship-metric-tools"><select class="relationship-metric-sort" :value="relationshipSortMode[relationshipMetricKey('agents', 'direct', 'firstDepositCount')] ?? ''" @click.stop @change="setRelationshipSort(relationshipMetricKey('agents', 'direct', 'firstDepositCount'), $event)"><option value="">排序</option><option value="firstDepositDesc">時間由近而遠</option></select><button type="button" class="relationship-metric-filter" :class="{ active: relationshipFilterEnabled[relationshipMetricKey('agents', 'direct', 'firstDepositCount')] }" @click.stop="toggleRelationshipFilter(relationshipMetricKey('agents', 'direct', 'firstDepositCount'))">{{ relationshipFilterEnabled[relationshipMetricKey('agents', 'direct', 'firstDepositCount')] ? '已篩選' : '篩選' }}</button></div></div><strong>{{ relationshipCycleCount('agents', 'direct') }} 位</strong></div><div class="relationship-clean-metric relationship-cycle-metric"><div class="relationship-metric-label"><small>結算週期首存總金額</small><div class="relationship-metric-tools"><select class="relationship-metric-sort" :value="relationshipSortMode[relationshipMetricKey('agents', 'direct', 'firstDepositAmount')] ?? ''" @click.stop @change="setRelationshipSort(relationshipMetricKey('agents', 'direct', 'firstDepositAmount'), $event)"><option value="">排序</option><option value="firstDepositAmountDesc">金額由大到小</option></select><button type="button" class="relationship-metric-filter" :class="{ active: relationshipFilterEnabled[relationshipMetricKey('agents', 'direct', 'firstDepositAmount')] }" @click.stop="toggleRelationshipFilter(relationshipMetricKey('agents', 'direct', 'firstDepositAmount'))">{{ relationshipFilterEnabled[relationshipMetricKey('agents', 'direct', 'firstDepositAmount')] ? '已篩選' : '篩選' }}</button></div></div><strong>{{ relationshipCycleAmount('agents', 'direct') }}</strong></div><div class="relationship-clean-metric relationship-cycle-metric"><div class="relationship-metric-label"><small>結算週期註冊人數</small><div class="relationship-metric-tools"><select class="relationship-metric-sort" :value="relationshipSortMode[relationshipMetricKey('agents', 'direct', 'registeredCount')] ?? ''" @click.stop @change="setRelationshipSort(relationshipMetricKey('agents', 'direct', 'registeredCount'), $event)"><option value="">排序</option><option value="registeredDesc">時間由近而遠</option></select><button type="button" class="relationship-metric-filter" :class="{ active: relationshipFilterEnabled[relationshipMetricKey('agents', 'direct', 'registeredCount')] }" @click.stop="toggleRelationshipFilter(relationshipMetricKey('agents', 'direct', 'registeredCount'))">{{ relationshipFilterEnabled[relationshipMetricKey('agents', 'direct', 'registeredCount')] ? '已篩選' : '篩選' }}</button></div></div><strong>{{ relationshipCycleRegistered('agents', 'direct') }} 位</strong></div></div></div>
              ...
              <div class="relationship-clean-row">
                <div class="relationship-clean-trigger" role="button" tabindex="0" @click="toggleRelationshipSection('totalPlayers')"><div class="relationship-clean-title-row"><span class="relationship-clean-title">總下級（含直屬）玩家</span><em>{{ relationshipExpanded.totalPlayers ? '收闔' : '展開' }}⌄</em></div><div class="relationship-clean-metrics"><div class="relationship-clean-metric"><small>玩家人數</small><strong>{{ relationshipRows('players', 'all').length }} 位</strong></div><div class="relationship-clean-metric"><small>總儲值金額</small><strong>{{ selectedAgent.currency }} {{ relationshipSum('players', 'all', 'deposit').toLocaleString() }}</strong></div><div class="relationship-clean-metric"><small>總有效投注</small><strong>{{ selectedAgent.currency }} {{ relationshipSum('players', 'all', 'bet').toLocaleString() }}</strong></div><div class="relationship-clean-metric relationship-cycle-metric"><div class="relationship-metric-label"><small>結算週期首存人數</small><div class="relationship-metric-tools"><select class="relationship-metric-sort" :value="relationshipSortMode[relationshipMetricKey('players', 'all', 'firstDepositCount')] ?? ''" @click.stop @change="setRelationshipSort(relationshipMetricKey('players', 'all', 'firstDepositCount'), $event)"><option value="">排序</option><option value="firstDepositDesc">時間由近而遠</option></select><button type="button" class="relationship-metric-filter" :class="{ active: relationshipFilterEnabled[relationshipMetricKey('players', 'all', 'firstDepositCount')] }" @click.stop="toggleRelationshipFilter(relationshipMetricKey('players', 'all', 'firstDepositCount'))">{{ relationshipFilterEnabled[relationshipMetricKey('players', 'all', 'firstDepositCount')] ? '已篩選' : '篩選' }}</button></div></div><strong>{{ relationshipCycleCount('players', 'all') }} 位</strong></div><div class="relationship-clean-metric relationship-cycle-metric"><div class="relationship-metric-label"><small>結算週期首存總金額</small><div class="relationship-metric-tools"><select class="relationship-metric-sort" :value="relationshipSortMode[relationshipMetricKey('players', 'all', 'firstDepositAmount')] ?? ''" @click.stop @change="setRelationshipSort(relationshipMetricKey('players', 'all', 'firstDepositAmount'), $event)"><option value="">排序</option><option value="firstDepositAmountDesc">金額由大到小</option></select><button type="button" class="relationship-metric-filter" :class="{ active: relationshipFilterEnabled[relationshipMetricKey('players', 'all', 'firstDepositAmount')] }" @click.stop="toggleRelationshipFilter(relationshipMetricKey('players', 'all', 'firstDepositAmount'))">{{ relationshipFilterEnabled[relationshipMetricKey('players', 'all', 'firstDepositAmount')] ? '已篩選' : '篩選' }}</button></div></div><strong>{{ relationshipCycleAmount('players', 'all') }}</strong></div><div class="relationship-clean-metric relationship-cycle-metric"><div class="relationship-metric-label"><small>結算週期註冊人數</small><div class="relationship-metric-tools"><select class="relationship-metric-sort" :value="relationshipSortMode[relationshipMetricKey('players', 'all', 'registeredCount')] ?? ''" @click.stop @change="setRelationshipSort(relationshipMetricKey('players', 'all', 'registeredCount'), $event)"><option value="">排序</option><option value="registeredDesc">時間由近而遠</option></select><button type="button" class="relationship-metric-filter" :class="{ active: relationshipFilterEnabled[relationshipMetricKey('players', 'all', 'registeredCount')] }" @click.stop="toggleRelationshipFilter(relationshipMetricKey('players', 'all', 'registeredCount'))">{{ relationshipFilterEnabled[relationshipMetricKey('players', 'all', 'registeredCount')] ? '已篩選' : '篩選' }}</button></div></div><strong>{{ relationshipCycleRegistered('players', 'all') }} 位</strong></div></div></div>
              ...
              <div class="relationship-clean-row">
                <div class="relationship-clean-trigger" role="button" tabindex="0" @click="toggleRelationshipSection('directPlayers')"><div class="relationship-clean-title-row"><span class="relationship-clean-title">直屬下級玩家</span><em>{{ relationshipExpanded.directPlayers ? '收闔' : '展開' }}⌄</em></div><div class="relationship-clean-metrics"><div class="relationship-clean-metric"><small>玩家人數</small><strong>{{ relationshipRows('players', 'direct').length }} 位</strong></div><div class="relationship-clean-metric"><small>總儲值金額</small><strong>{{ selectedAgent.currency }} {{ relationshipSum('players', 'direct', 'deposit').toLocaleString() }}</strong></div><div class="relationship-clean-metric"><small>總有效投注</small><strong>{{ selectedAgent.currency }} {{ relationshipSum('players', 'direct', 'bet').toLocaleString() }}</strong></div><div class="relationship-clean-metric relationship-cycle-metric"><div class="relationship-metric-label"><small>結算週期首存人數</small><div class="relationship-metric-tools"><select class="relationship-metric-sort" :value="relationshipSortMode[relationshipMetricKey('players', 'direct', 'firstDepositCount')] ?? ''" @click.stop @change="setRelationshipSort(relationshipMetricKey('players', 'direct', 'firstDepositCount'), $event)"><option value="">排序</option><option value="firstDepositDesc">時間由近而遠</option></select><button type="button" class="relationship-metric-filter" :class="{ active: relationshipFilterEnabled[relationshipMetricKey('players', 'direct', 'firstDepositCount')] }" @click.stop="toggleRelationshipFilter(relationshipMetricKey('players', 'direct', 'firstDepositCount'))">{{ relationshipFilterEnabled[relationshipMetricKey('players', 'direct', 'firstDepositCount')] ? '已篩選' : '篩選' }}</button></div></div><strong>{{ relationshipCycleCount('players', 'direct') }} 位</strong></div><div class="relationship-clean-metric relationship-cycle-metric"><div class="relationship-metric-label"><small>結算週期首存總金額</small><div class="relationship-metric-tools"><select class="relationship-metric-sort" :value="relationshipSortMode[relationshipMetricKey('players', 'direct', 'firstDepositAmount')] ?? ''" @click.stop @change="setRelationshipSort(relationshipMetricKey('players', 'direct', 'firstDepositAmount'), $event)"><option value="">排序</option><option value="firstDepositAmountDesc">金額由大到小</option></select><button type="button" class="relationship-metric-filter" :class="{ active: relationshipFilterEnabled[relationshipMetricKey('players', 'direct', 'firstDepositAmount')] }" @click.stop="toggleRelationshipFilter(relationshipMetricKey('players', 'direct', 'firstDepositAmount'))">{{ relationshipFilterEnabled[relationshipMetricKey('players', 'direct', 'firstDepositAmount')] ? '已篩選' : '篩選' }}</button></div></div><strong>{{ relationshipCycleAmount('players', 'direct') }}</strong></div><div class="relationship-clean-metric relationship-cycle-metric"><div class="relationship-metric-label"><small>結算週期註冊人數</small><div class="relationship-metric-tools"><select class="relationship-metric-sort" :value="relationshipSortMode[relationshipMetricKey('players', 'direct', 'registeredCount')] ?? ''" @click.stop @change="setRelationshipSort(relationshipMetricKey('players', 'direct', 'registeredCount'), $event)"><option value="">排序</option><option value="registeredDesc">時間由近而遠</option></select><button type="button" class="relationship-metric-filter" :class="{ active: relationshipFilterEnabled[relationshipMetricKey('players', 'direct', 'registeredCount')] }" @click.stop="toggleRelationshipFilter(relationshipMetricKey('players', 'direct', 'registeredCount'))">{{ relationshipFilterEnabled[relationshipMetricKey('players', 'direct', 'registeredCount')] ? '已篩選' : '篩選' }}</button></div></div><strong>{{ relationshipCycleRegistered('players', 'direct') }} 位</strong></div></div></div>
      ...
      <NModal v-model:show="showAgentDetailModal" preset="card" :title="selectedAgent ? `代理詳情 · ${selectedAgent.account}` : '代理詳情'" class="modal-card">
        <template v-if="selectedAgent">
           <div class="detail-tabs"><button :class="{ active: detailTab === 'basic' }" @click="detailTab = 'basic'">基本資料</button><button :class="{ active: detailTab === 'wallet' }" @click="detailTab = 'wallet'">即時資料</button><button :class="{ active: detailTab === 'relationship' }" @click="detailTab = 'relationship'">代理報表</button><button :class="{ active: detailTab === 'commission' }" @click="detailTab = 'commission'">傭金設定</button><button :class="{ active: detailTab === 'withdrawals' }" @click="detailTab = 'withdrawals'">傭金帳變紀錄</button><button :class="{ active: detailTab === 'logs' }" @click="detailTab = 'logs'">操作紀錄</button></div>
