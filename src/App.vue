...
const agentColumns = [
  ...
  { title: '可分配反傭', key: 'point', render: (row: AgentRow) => `${row.point}%` },
        ...
        <NLayout>
          ...
          <main class="content-area">
            ...
            <section v-else-if="activeKey === 'agents'">
              <div class="section-head"><div><h1>代理管理</h1><p class="muted">查看代理樹狀關係；代理可立即開設下級，僅能調整反傭比例。</p></div><NButton type="primary" @click="showCreateAgent = true">＋ 開設下級代理</NButton></div>
            ...
            <section v-else-if="activeKey === 'codes'">
              ...
              <div class="code-grid"><NCard title="直屬代理推廣碼" class="code-card"><p class="muted">下線註冊成為代理後，可依代理線規則產生傭金。</p><div class="code-box">AGT-TW-8F4K-29Q1<NButton quaternary circle><NIcon><CopyOutline /></NIcon></NButton></div><NTag type="success" round>可產生代理傭金</NTag></NCard><NCard title="直屬玩家推廣碼" class="code-card"><p class="muted">僅用於玩家註冊，不產生下線抽傭；註冊後不再建立代理推廣碼。</p><div class="code-box">PLY-TW-3M7X-62ZD<NButton quaternary circle><NIcon><CopyOutline /></NIcon></NButton></div><NTag round>不產生代理傭金</NTag></NCard></div>
            ...
            <section v-else-if="activeKey === 'reports'">
              <div class="section-head"><div><h1>下級報表</h1><p class="muted">以樹狀路徑與層級欄位呈現，避免無限層代理造成數據重複。</p></div><NButton secondary>匯出報表</NButton></div>
            ...
            <section v-else-if="activeKey === 'logs'">
              <div class="section-head"><div><h1>操作日誌</h1><p class="muted">完整保留登入、開設代理、調整反傭、提領等操作。</p></div><NButton secondary>匯出紀錄</NButton></div>
