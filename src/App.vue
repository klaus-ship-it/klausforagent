...
function relationshipDetailRows(type: 'agents' | 'players', scope: 'all' | 'direct') {
  ...
  if (relationshipFilterScope.value === 'cycle') rows = rows.filter((row) => relationshipInCycle(relationshipFirstDeposit(row)?.at) || relationshipInCycle(relationshipRegisterAt(row)))
