...
const commissionPlans = ref<CommissionPlan[]>([
  { id: 'PLAN-001', name: '台灣返佣標準方案', promoCode: 'PLAN-TW-8F4K', createdBy: 'operator_demo（運營商）', parentAccount: 'agent_taipei（總代理）', model: '返佣', cycle: '每週', allocationRate: 6, description: '依有效投注額計算返佣；下級可用點數不得超過本方案額度。', assignedCount: 3, status: '啟用', config: { ...defaultCommissionConfig(), rebate: { rebateRate: 4, tieredEnabled: true } } },
  ...
  { id: 'PLAN-004', name: '北區下級返佣方案', promoCode: 'PLAN-TW-NORTH4', createdBy: 'north_team（一級代理）', parentAccount: 'north_team（一級代理）', model: '返佣', cycle: '每週', allocationRate: 4, description: '北區代理線下級方案；點數不得高於上級提供額度。', assignedCount: 1, status: '啟用', config: { ...defaultCommissionConfig(), rebate: { rebateRate: 3, tieredEnabled: false } } },
])
