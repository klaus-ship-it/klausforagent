...
const commissionPlans = ref<CommissionPlan[]>([
  { id: 'PLAN-001', name: '台灣返佣標準方案', promoCode: 'PLAN-TW-8F4K', createdBy: 'operator_demo（運營商）', parentAccount: 'agent_taipei（總代理）', model: '返佣', cycle: '每週', allocationRate: 6, description: '依有效投注額計算返佣；下級可用點數不得超過本方案額度。', assignedCount: 3, status: '啟用', config: { ...defaultCommissionConfig(), rebate: { rebateRate: 4 } } },
  { id: 'PLAN-002', name: '台北總代直屬方案', promoCode: 'PLAN-TW-DIRECT2', createdBy: 'agent_taipei（總代理）', parentAccount: 'agent_taipei（總代理）', model: '返佣', cycle: '每週', allocationRate: 5, description: '總代理提供給直屬下級的返佣方案；下級可依額度建立自己的方案。', assignedCount: 2, status: '啟用', config: { ...defaultCommissionConfig(), rebate: { rebateRate: 3.5 } } },
  { id: 'PLAN-003', name: '佔成合作方案', promoCode: 'PLAN-TW-PNL3', createdBy: 'operator_demo（運營商）', parentAccount: 'agent_taipei（總代理）', model: '佔成', cycle: '每月', allocationRate: 5, description: '依玩家總輸贏扣除行政成本後按比例分配，支援負數沖銷或負數累計。', assignedCount: 0, status: '啟用', config: { ...defaultCommissionConfig(), share: { shareRate: 5, adminCostRate: 1, negativeMode: '負數沖銷', offsetType: '週期固定沖銷額度', offsetLimit: 10000 } } },
  { id: 'PLAN-004', name: '北區下級返佣方案', promoCode: 'PLAN-TW-NORTH4', createdBy: 'north_team（一級代理）', parentAccount: 'north_team（一級代理）', model: '返佣', cycle: '每週', allocationRate: 4, description: '北區代理線下級方案；點數不得高於上級提供額度。', assignedCount: 1, status: '啟用', config: { ...defaultCommissionConfig(), rebate: { rebateRate: 3 } } },
  { id: 'PLAN-005', name: '北區進階返佣方案', promoCode: 'PLAN-TW-NORTH5', createdBy: 'north_team（一級代理）', parentAccount: 'north_team（一級代理）', model: '返佣', cycle: '每週', allocationRate: 3, description: '北區二級代理使用的固定返佣方案，修改於下個結算週期生效。', assignedCount: 1, status: '啟用', config: { ...defaultCommissionConfig(), rebate: { rebateRate: 2.5 } } },
])
...
const filteredVisiblePlans = computed(() => visiblePlans.value.filter((plan) => {
  const promoQuery = planReportPromoCode.value.trim().toLowerCase()
  ...
  return (!promoQuery || plan.promoCode.toLowerCase().includes(promoQuery)) && (!creatorQuery || creator.includes(creatorQuery)) && hasAppliedAccount && (!planReportModel.value || plan.model === planReportModel.value) && (!planReportCycle.value || plan.cycle === planReportCycle.value) && (!planReportStatus.value || plan.status === planReportStatus.value)
}))
...
function savePlan() {
  ...
  if (editingPlan.value) {
    ...
    commissionPlans.value.push({ id, name: planName.value.trim(), promoCode: planPromoCode.value.trim(), createdBy: `${identity.value.account}（${identity.value.label}）`, parentAccount: planParentAccount.value.trim(), model: planModel.value, cycle: planCycle.value, allocationRate: planRate.value, description: planDescription.value.trim() || '依此方案規則產生代理傭金。', assignedCount: 0, status: '啟用', config })
          ...
          <main class="content-area">
            ...
            <section v-else-if="activeKey === 'agents'">
              ...
              <template v-else-if="selectedAgent">
                ...
                <div v-else-if="detailTab === 'commission'" class="agent-plan-overview detail-page-card"><div class="agent-plan-overview-head"><div><span>上級給予此代理的方案</span><strong>{{ selectedAgentPlan?.name || '未套用方案' }}</strong></div><NTag type="info" round>唯讀查看</NTag></div><div v-if="selectedAgentPlan" class="agent-plan-overview-grid"><div><span>方案推廣碼</span><strong class="code-text">{{ selectedAgentPlan.promoCode }}</strong></div><div><span>建立代理</span><strong>{{ selectedAgentPlan.createdBy || selectedAgentPlan.parentAccount }}</strong></div><div><span>傭金模式</span><strong>{{ selectedAgentPlan.model }}</strong></div><div><span>結算週期</span><strong>{{ selectedAgentPlan.cycle }}</strong></div><div><span>默認可分配點數</span><strong>{{ selectedAgentPlan.allocationRate }}%</strong></div><div><span>方案規則</span><strong>{{ selectedAgentPlan.description }}</strong></div></div><div class="agent-plan-overview-section"><div class="table-section-label"><strong>此代理建立給直屬下級的方案</strong><span>僅顯示方案與套用代理，新增／修改請至傭金方案設定</span></div><div v-for="plan in selectedAgentCreatedPlans" :key="plan.id" class="agent-downline-plan-row"><div><strong>{{ plan.name }}</strong><small class="code-text">{{ plan.promoCode }} · {{ plan.model }} · {{ plan.cycle }}</small></div><strong>{{ plan.allocationRate }}%</strong><span>套用 {{ planAppliedAgents(plan).length }} 位</span><div class="agent-downline-plan-agents"><NTag v-for="agent in planAppliedAgents(plan)" :key="agent.id" size="small" round :type="agent.status === '啟用' ? 'success' : 'error'">{{ agent.account }}</NTag><span v-if="!planAppliedAgents(plan).length" class="muted">尚無套用代理</span></div></div><p v-if="!selectedAgentCreatedPlans.length" class="modal-help">此代理尚未建立給直屬下級的方案。</p></div></div>
            ...
            <section v-else-if="activeKey === 'codes'">
              ...
              <template v-if="currentRole !== '運營商'"><div class="table-section-label"><strong>上級開給我的方案</strong><span>上級提供的方案僅供查看，不可修改</span></div><NCard :bordered="false" class="table-card upstream-plan-card"><div v-if="currentUpstreamPlan" class="upstream-plan-row"><div><span>方案名稱</span><strong>{{ currentUpstreamPlan.name }}</strong></div><div><span>方案推廣碼</span><code>{{ currentUpstreamPlan.promoCode }}</code></div><div><span>建立代理</span><strong>{{ currentUpstreamPlan.createdBy || currentUpstreamPlan.parentAccount }}</strong></div><div><span>傭金模式</span><NTag size="small" round :type="currentUpstreamPlan.model === '佔成' ? 'warning' : 'success'">{{ currentUpstreamPlan.model }}</NTag></div><div><span>結算週期</span><strong>{{ currentUpstreamPlan.cycle }}</strong></div><div><span>默認點數</span><strong>{{ currentUpstreamPlan.allocationRate }}%</strong></div><div><span>套用代理</span><strong>{{ planAppliedAgents(currentUpstreamPlan).length }} 位</strong></div><div><span>狀態</span><NTag size="small" round type="success">{{ currentUpstreamPlan.status }}</NTag></div></div><p v-else class="modal-help">目前尚未有上級方案。</p></NCard></template>
              <div class="table-section-label"><strong>我開給直屬下級的方案</strong><span>此區塊只顯示自己建立的方案；套用代理可展開查看明細</span></div><NCard v-if="currentRole !== '一般代理'" class="filter-card filter-card-emphasis plan-filter-card"><div class="filter-row plan-filter-row"><div class="filter-field-group"><label>傭金模式</label><NSelect v-model:value="codesPlanModelFilter" clearable placeholder="全部模式" :options="[{label:'佔成',value:'佔成'},{label:'返佣',value:'返佣'}]" /></div><div class="filter-field-group"><label>狀態</label><NSelect v-model:value="codesPlanStatusFilter" clearable placeholder="全部狀態" :options="[{label:'啟用',value:'啟用'},{label:'停用',value:'停用'}]" /></div></div></NCard>
              <NCard :bordered="false" class="table-card"><div class="plan-table-wrap"><div class="plan-table plan-table-own"><div class="plan-table-head"><span>方案名稱</span><span>方案推廣碼</span><span>傭金模式</span><span>結算週期</span><span>默認點數</span><span>套用代理</span><span>狀態</span><span>操作</span></div><div v-for="plan in filteredCurrentCreatedPlans" :key="plan.id" class="plan-table-row"><div><strong>{{ plan.name }}</strong><small>{{ plan.description }}</small></div><span class="code-text">{{ plan.promoCode }}</span><div><NTag size="small" :type="plan.model === '佔成' ? 'warning' : 'success'" round>{{ plan.model }}</NTag><small class="mode-summary">{{ planConfigSummary(plan) }}</small></div><span>{{ plan.cycle }}</span><span>{{ plan.allocationRate }}%</span><span class="plan-assignee-count">{{ planAppliedAgents(plan).length }} 位<NButton quaternary size="tiny" class="plan-expand-inline" @click="togglePlanExpanded(plan.id)">{{ expandedPlanIds.includes(plan.id) ? '收闔' : '展開' }}</NButton></span><NTag size="small" :type="plan.status === '啟用' ? 'success' : 'error'" round>{{ plan.status }}</NTag><div class="table-actions"><NButton v-if="canEditPlan(plan)" quaternary size="small" @click="openPlanEditor(plan)">編輯</NButton><NButton quaternary size="small" @click="copyReferral(plan.promoCode, '方案推廣碼')">複製碼</NButton><NButton v-if="canEditPlan(plan)" quaternary size="small" @click="togglePlan(plan)">{{ plan.status === '啟用' ? '停用' : '啟用' }}</NButton></div><div v-if="expandedPlanIds.includes(plan.id)" class="plan-assigned-detail"><div class="plan-assigned-head"><span>套用代理帳號</span><span>狀態</span><span>註冊時間</span><span>最後登入時間</span></div><div v-for="agent in planAppliedAgents(plan)" :key="agent.id" class="plan-assigned-row"><strong>{{ agent.account }}</strong><NTag size="small" round :type="agent.status === '啟用' ? 'success' : 'error'">{{ agent.status }}</NTag><time>{{ agent.registerAt || '-' }}</time><time>{{ agent.lastLoginAt || '-' }}</time></div><p v-if="!planAppliedAgents(plan).length" class="modal-help">目前沒有代理套用此方案。</p></div></div><p v-if="!filteredCurrentCreatedPlans.length" class="modal-help">目前尚未建立給直屬下級的方案。</p></div></div></NCard>
            </section>

            <section v-else-if="activeKey === 'commissionPlans'">
              <div class="section-head"><div><h1>傭金方案</h1><p class="muted">報表方式查看目前角色可見的傭金方案與對應推廣碼，並展開查看套用代理。</p><p class="line-rule">{{ currentRole === '運營商' ? '運營商可查看與管理所有代理線方案。' : `目前顯示上級提供及本代理線可見方案；方案調整請至傭金方案設定。` }}</p></div></div>
               <div class="plan-mode-grid"><NCard v-for="mode in [{name:'佔成',desc:'依玩家總輸贏扣除行政成本後按比例分配，支援負數規則。'},{name:'返佣',desc:'依有效投注額按固定返佣比例計算。'}]" :key="mode.name" class="plan-mode-card"><NTag :type="mode.name === '佔成' ? 'warning' : 'success'" round>{{ mode.name }}</NTag><span>{{ mode.desc }}</span></NCard></div>
<NCard v-if="currentRole !== '一般代理'" class="filter-card filter-card-emphasis plan-filter-card plan-report-filter-card"><div class="filter-row plan-filter-row"><div class="filter-field-group"><label>方案推廣碼</label><NInput v-model:value="planReportPromoCode" clearable placeholder="查詢推廣碼" /></div><div class="filter-field-group"><label>建立代理</label><NInput v-model:value="planReportCreatedBy" clearable placeholder="查詢建立代理" /></div><div class="filter-field-group"><label>套用帳號</label><NInput v-model:value="planReportAppliedAccount" clearable placeholder="查詢套用帳號" /></div><div class="filter-field-group"><label>傭金模式</label><NSelect v-model:value="planReportModel" clearable placeholder="全部模式" :options="[{label:'佔成',value:'佔成'},{label:'返佣',value:'返佣'}]" /></div><div class="filter-field-group"><label>結算週期</label><NSelect v-model:value="planReportCycle" clearable placeholder="全部週期" :options="[{label:'即時',value:'即時'},{label:'每日',value:'每日'},{label:'每週',value:'每週'},{label:'每月',value:'每月'}]" /></div><div class="filter-field-group"><label>狀態</label><NSelect v-model:value="planReportStatus" clearable placeholder="全部狀態" :options="[{label:'啟用',value:'啟用'},{label:'停用',value:'停用'}]" /></div></div></NCard>
               <div class="table-section-label"><strong>方案資料</strong><span>建立代理代表方案建立者；展開後查看套用代理帳號、狀態、註冊時間與最後登入時間</span></div><NCard :bordered="false" class="table-card"><div class="plan-table-wrap"><div class="plan-table"><div class="plan-table-head"><span>方案名稱</span><span>方案推廣碼</span><span>建立代理</span><span>傭金模式</span><span>結算週期</span><span>默認點數</span><span>套用代理</span><span>狀態</span><span>操作</span></div><div v-for="plan in filteredVisiblePlans" :key="plan.id" class="plan-table-row"><div><strong>{{ plan.name }}</strong><small>{{ plan.description }}</small></div><span class="code-text">{{ plan.promoCode }}</span><span>{{ plan.createdBy || plan.parentAccount }}</span><div><NTag size="small" :type="plan.model === '佔成' ? 'warning' : 'success'" round>{{ plan.model }}</NTag><small class="mode-summary">{{ planConfigSummary(plan) }}</small></div><span>{{ plan.cycle }}</span><span>{{ plan.allocationRate }}%</span><span class="plan-assignee-count">{{ planAppliedAgents(plan).length }} 位<NButton quaternary size="tiny" class="plan-expand-inline" @click="togglePlanExpanded(plan.id)">{{ expandedPlanIds.includes(plan.id) ? '收闔' : '展開' }}</NButton></span><NTag size="small" :type="plan.status === '啟用' ? 'success' : 'error'" round>{{ plan.status }}</NTag><div class="table-actions"><NButton v-if="canEditPlan(plan)" quaternary size="small" @click="openPlanEditor(plan)">編輯</NButton><NButton quaternary size="small" @click="copyReferral(plan.promoCode, '方案推廣碼')">複製碼</NButton><NButton v-if="canEditPlan(plan)" quaternary size="small" @click="togglePlan(plan)">{{ plan.status === '啟用' ? '停用' : '啟用' }}</NButton><span v-if="!canEditPlan(plan)" class="muted">上級唯讀</span></div><div v-if="expandedPlanIds.includes(plan.id)" class="plan-assigned-detail"><div class="plan-assigned-head"><span>套用代理帳號</span><span>狀態</span><span>註冊時間</span><span>最後登入時間</span></div><div v-for="agent in planAppliedAgents(plan)" :key="agent.id" class="plan-assigned-row"><strong>{{ agent.account }}</strong><NTag size="small" round :type="agent.status === '啟用' ? 'success' : 'error'">{{ agent.status }}</NTag><time>{{ agent.registerAt || '-' }}</time><time>{{ agent.lastLoginAt || '-' }}</time></div><p v-if="!planAppliedAgents(plan).length" class="modal-help">目前沒有代理套用此方案。</p></div></div></div></div><div class="table-hint">每個方案推廣碼都固定記錄建立代理、傭金模式、結算週期與默認點數；同一條代理線只能使用一種傭金模式，下級方案不得超過上級額度。</div></NCard>
            </section>
          ...
          <div v-if="activeKey === 'agents' && showAgentDetail && detailTab === 'commission' && selectedAgent" class="relationship-clean-panel commission-plan-overlay">
            ...
            <NCard :bordered="false" class="upstream-plan-card"><div v-if="selectedAgentPlan" class="upstream-plan-row"><div><span>方案名稱</span><strong>{{ selectedAgentPlan.name }}</strong></div><div><span>方案推廣碼</span><code>{{ selectedAgentPlan.promoCode }}</code></div><div><span>建立代理</span><strong>{{ selectedAgentPlan.createdBy || selectedAgentPlan.parentAccount }}</strong></div><div><span>傭金模式</span><NTag size="small" round :type="selectedAgentPlan.model === '佔成' ? 'warning' : 'success'">{{ selectedAgentPlan.model }}</NTag></div><div><span>結算週期</span><strong>{{ selectedAgentPlan.cycle }}</strong></div><div><span>默認點數</span><strong>{{ selectedAgentPlan.allocationRate }}%</strong></div><div><span>套用代理</span><strong>{{ planAppliedAgents(selectedAgentPlan).length }} 位</strong></div><div><span>狀態</span><NTag size="small" round type="success">{{ selectedAgentPlan.status }}</NTag></div></div><p v-else class="modal-help">目前尚未有上級方案。</p></NCard>
            <div class="table-section-label"><strong>我開給直屬下級的方案</strong><span>此區塊不顯示建立代理與操作欄位；套用代理可展開查看明細</span></div><NCard v-if="currentRole !== '一般代理'" class="filter-card filter-card-emphasis plan-filter-card"><div class="filter-row plan-filter-row"><div class="filter-field-group"><label>傭金模式</label><NSelect v-model:value="detailPlanModelFilter" clearable placeholder="全部模式" :options="[{label:'佔成',value:'佔成'},{label:'返佣',value:'返佣'}]" /></div><div class="filter-field-group"><label>狀態</label><NSelect v-model:value="detailPlanStatusFilter" clearable placeholder="全部狀態" :options="[{label:'啟用',value:'啟用'},{label:'停用',value:'停用'}]" /></div></div></NCard>
            <NCard :bordered="false" class="table-card"><div class="plan-table-wrap"><div class="plan-table plan-table-own"><div class="plan-table-head"><span>方案名稱</span><span>方案推廣碼</span><span>傭金模式</span><span>結算週期</span><span>默認點數</span><span>套用代理</span><span>狀態</span></div><div v-for="plan in filteredSelectedAgentCreatedPlans" :key="plan.id" class="plan-table-row"><div><strong>{{ plan.name }}</strong><small>{{ plan.description }}</small></div><span class="code-text">{{ plan.promoCode }}</span><div><NTag size="small" :type="plan.model === '佔成' ? 'warning' : 'success'" round>{{ plan.model }}</NTag><small class="mode-summary">{{ planConfigSummary(plan) }}</small></div><span>{{ plan.cycle }}</span><span>{{ plan.allocationRate }}%</span><span class="plan-assignee-count">{{ planAppliedAgents(plan).length }} 位<NButton quaternary size="tiny" class="plan-expand-inline" @click="togglePlanExpanded(plan.id)">{{ expandedPlanIds.includes(plan.id) ? '收闔' : '展開' }}</NButton></span><NTag size="small" :type="plan.status === '啟用' ? 'success' : 'error'" round>{{ plan.status }}</NTag><div v-if="expandedPlanIds.includes(plan.id)" class="plan-assigned-detail"><div class="plan-assigned-head"><span>套用代理帳號</span><span>狀態</span><span>註冊時間</span><span>最後登入時間</span></div><div v-for="agent in planAppliedAgents(plan)" :key="agent.id" class="plan-assigned-row"><strong>{{ agent.account }}</strong><NTag size="small" round :type="agent.status === '啟用' ? 'success' : 'error'">{{ agent.status }}</NTag><time>{{ agent.registerAt || '-' }}</time><time>{{ agent.lastLoginAt || '-' }}</time></div><p v-if="!planAppliedAgents(plan).length" class="modal-help">目前沒有代理套用此方案。</p></div></div><p v-if="!filteredSelectedAgentCreatedPlans.length" class="modal-help">此代理尚未建立給直屬下級的方案。</p></div></div></NCard>
          </div>
      ...
      <NModal v-model:show="showAgentDetailModal" preset="card" :title="selectedAgent ? `代理詳情 · ${selectedAgent.account}` : '代理詳情'" class="modal-card">
        <template v-if="selectedAgent">
           ...
           <div v-else-if="detailTab === 'commission'" class="agent-plan-overview"><div class="agent-plan-overview-head"><div><span>上級給予此代理的方案</span><strong>{{ selectedAgentPlan?.name || '未套用方案' }}</strong></div><NTag type="info" round>唯讀查看</NTag></div><div v-if="selectedAgentPlan" class="agent-plan-overview-grid"><div><span>方案推廣碼</span><strong class="code-text">{{ selectedAgentPlan.promoCode }}</strong></div><div><span>建立代理</span><strong>{{ selectedAgentPlan.createdBy || selectedAgentPlan.parentAccount }}</strong></div><div><span>傭金模式</span><strong>{{ selectedAgentPlan.model }}</strong></div><div><span>結算週期</span><strong>{{ selectedAgentPlan.cycle }}</strong></div><div><span>默認可分配點數</span><strong>{{ selectedAgentPlan.allocationRate }}%</strong></div><div><span>方案規則</span><strong>{{ selectedAgentPlan.description }}</strong></div></div><div class="agent-plan-overview-section"><strong>此代理建立給直屬下級的方案</strong><div v-for="plan in selectedAgentCreatedPlans" :key="plan.id" class="agent-downline-plan-row"><div><strong>{{ plan.name }}</strong><small class="code-text">{{ plan.promoCode }} · {{ plan.model }} · {{ plan.cycle }}</small></div><strong>{{ plan.allocationRate }}%</strong><span>套用 {{ planAppliedAgents(plan).length }} 位</span></div><p v-if="!selectedAgentCreatedPlans.length" class="modal-help">此代理尚未建立給直屬下級的方案。</p></div></div>
