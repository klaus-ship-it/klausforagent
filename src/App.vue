...
interface CommissionConfig {
  cpa: { registerFixed: number; firstDepositFixed: number; firstDepositMin: number; turnoverFixed: number; turnoverMin: number; depositRate: number }
...
const cpaRegisterFixed = ref<number | null>(500)
const cpaFirstDepositFixed = ref<number | null>(1000)
const cpaFirstDepositMin = ref<number | null>(300)
const cpaTurnoverFixed = ref<number | null>(1500)
const cpaTurnoverMin = ref<number | null>(3000)
const cpaDepositRate = ref<number | null>(2)
...
function defaultCommissionConfig(): CommissionConfig {
  return {
    cpa: { registerFixed: 500, firstDepositFixed: 1000, firstDepositMin: 300, turnoverFixed: 1500, turnoverMin: 3000, depositRate: 2 },
...
function planConfig(plan: CommissionPlan): CommissionConfig {
  ...
  return {
    cpa: { ...fallback.cpa, ...plan.config?.cpa },
...
function planConfigSummary(plan: CommissionPlan) {
  ...
  if (plan.model === 'CPA') return `註冊 $${config.cpa.registerFixed}／首存 $${config.cpa.firstDepositFixed}／流水 $${config.cpa.turnoverFixed}`
...
const agentRows = ref<AgentRow[]>([
  ...
  { id: 'A-1005', uid: 'AG-10120', account: 'agent_cpa', displayName: 'CPA 總代理', referralCode: 'AGT-USD-CPA1', phone: '0911222333', email: 'cpa@example.com', contactMethod: 'Email', twoFactor: '已啟用', level: '總代理', path: 'agent_cpa', currency: 'USD', point: 4, children: 12, walletBalance: 18200, status: '啟用', planId: 'PLAN-002', model: 'CPA', cycle: '每月' },
...
const commissionPlans = ref<CommissionPlan[]>([
  ...
  { id: 'PLAN-002', name: 'CPA 新客獎勵方案', promoCode: 'PLAN-TW-CPA1', parentAccount: 'agent_taipei（總代理）', model: 'CPA', cycle: '每月', allocationRate: 4, description: '註冊、首存、流水達標各自設定固定獎勵，另按下線總儲值比例計算。', assignedCount: 0, status: '啟用', config: { ...defaultCommissionConfig(), cpa: { registerFixed: 500, firstDepositFixed: 1000, firstDepositMin: 300, turnoverFixed: 1500, turnoverMin: 3000, depositRate: 2 } } },
...
function openPlanEditor(plan?: CommissionPlan) {
  ...
  cpaRegisterFixed.value = config.cpa.registerFixed
  cpaFirstDepositFixed.value = config.cpa.firstDepositFixed
  cpaFirstDepositMin.value = config.cpa.firstDepositMin
  cpaTurnoverFixed.value = config.cpa.turnoverFixed
  cpaTurnoverMin.value = config.cpa.turnoverMin
  cpaDepositRate.value = config.cpa.depositRate
...
function savePlan() {
  ...
  if (planModel.value === 'CPA' && [cpaRegisterFixed.value, cpaFirstDepositFixed.value, cpaFirstDepositMin.value, cpaTurnoverFixed.value, cpaTurnoverMin.value, cpaDepositRate.value].some((value) => value === null || value < 0)) {
    showNotice('warning', 'CPA 設定未完成', '請完成註冊、首存、流水的固定金額／門檻，以及儲值抽成比例。')
    return
  }
  ...
  const config: CommissionConfig = {
    cpa: { registerFixed: cpaRegisterFixed.value ?? 0, firstDepositFixed: cpaFirstDepositFixed.value ?? 0, firstDepositMin: cpaFirstDepositMin.value ?? 0, turnoverFixed: cpaTurnoverFixed.value ?? 0, turnoverMin: cpaTurnoverMin.value ?? 0, depositRate: cpaDepositRate.value ?? 0 },
    ...
    <div>
      ...
      <NModal v-model:show="showAgentDetail" preset="card" :title="selectedAgent ? `代理詳情 · ${selectedAgent.account}` : '代理詳情'" class="modal-card">
        <template v-if="selectedAgent">
           ...
           <div v-else-if="detailTab === 'commission'" class="agent-detail-grid"><div class="full"><span>套用傭金方案</span><NSelect v-model:value="draftPlanId" :options="planOptions" /></div><div><span>方案推廣碼</span><strong class="code-text">{{ selectedPlan.promoCode }}</strong></div><div><span>註冊上級</span><strong>{{ selectedPlan.parentAccount }}</strong></div><div><span>傭金模式</span><strong>{{ selectedPlan.model }}</strong></div><div><span>結算週期</span><strong>{{ selectedPlan.cycle }}</strong></div><div><span>默認可分配點數</span><strong>{{ selectedPlan.allocationRate }}%</strong></div><div v-if="selectedPlan.model === 'CPA'" class="full"><span>CPA 設定</span><p class="modal-help">註冊固定 {{ selectedPlanConfig.cpa.registerFixed }}；首存達標固定 {{ selectedPlanConfig.cpa.firstDepositFixed }}（最低儲值 {{ selectedPlanConfig.cpa.firstDepositMin }}）；流水達標固定 {{ selectedPlanConfig.cpa.turnoverFixed }}（最低流水 {{ selectedPlanConfig.cpa.turnoverMin }}）；下線總儲值抽成 {{ selectedPlanConfig.cpa.depositRate }}%。</p></div><div v-else-if="selectedPlan.model === '佔成'" class="full"><span>佔成設定</span><p class="modal-help">玩家總輸贏扣除行政費率 {{ selectedPlanConfig.share.adminCostRate }}% 後，按 {{ selectedPlanConfig.share.shareRate }}% 分配；負數採{{ selectedPlanConfig.share.negativeMode }}，{{ selectedPlanConfig.share.offsetType }}{{ selectedPlanConfig.share.offsetLimit ? ` ${selectedPlanConfig.share.offsetLimit}` : '' }}。</p></div><div v-else class="full"><span>返佣設定</span><p class="modal-help">有效投注額 × {{ selectedPlanConfig.rebate.rebateRate }}%；{{ selectedPlanConfig.rebate.tieredEnabled ? '已啟用階梯式比例預留。' : '固定比例計算。' }}</p></div><div class="full"><span>方案規則</span><p class="modal-help">{{ selectedPlan.description }} 下級代理可設定的點數不得超過上級可分配額度。</p></div></div>
