...
const forceTwoFactorRebind = ref<Record<string, boolean>>({})
...
const agentRows = ref<AgentRow[]>([
  { id: 'A-1001', uid: 'AG-10001', account: 'agent_taipei', displayName: '台北總代理', referralCode: 'AGT-TW-8F4K', phone: '0912345678', email: 'taipei@example.com', contactMethod: 'Line: @agent_taipei', twoFactor: '已啟用', twoFactorBoundAt: '2026-07-18 14:22:08', twoFactorRequired: true, level: '總代理', path: 'agent_taipei', currency: 'TWD', point: 6, children: 3, walletBalance: 28460, status: '啟用', planId: 'PLAN-001', model: '返佣', cycle: '每週' },
  { id: 'A-1006', uid: 'AG-10121', account: 'agent_pnl', displayName: '佔成總代理', referralCode: 'AGT-TWD-PNL3', phone: '0922333444', email: 'pnl@example.com', contactMethod: 'Line: @agent_pnl', twoFactor: '未啟用', twoFactorBoundAt: '尚未綁定', twoFactorRequired: true, level: '總代理', path: 'agent_pnl', currency: 'TWD', point: 5, children: 6, walletBalance: 9740, status: '啟用', planId: 'PLAN-003', model: '佔成', cycle: '每月' },
  { id: 'A-1002', uid: 'AG-10024', account: 'north_team', displayName: '北區團隊', referralCode: 'AGT-TW-NORTH', phone: '0987654321', email: 'north@example.com', contactMethod: 'Line: @north_team', twoFactor: '未啟用', twoFactorRequired: true, level: '一級代理', path: 'agent_taipei > north_team', currency: 'TWD', point: 4, children: 8, walletBalance: 12680, status: '啟用', planId: 'PLAN-001', model: '返佣', cycle: '每週' },
  { id: 'A-1003', uid: 'AG-10088', account: 'east_team', displayName: '東區團隊', referralCode: 'AGT-TW-EAST', phone: '0955123788', email: 'east@example.com', contactMethod: 'Email', twoFactor: '未啟用', twoFactorRequired: true, level: '一級代理', path: 'agent_taipei > east_team', currency: 'TWD', point: 3, children: 5, walletBalance: 8340, status: '啟用', planId: 'PLAN-001', model: '返佣', cycle: '每週' },
...
function login() {
  ...
  if (forceTwoFactorRebind.value[loginAccount.value]) {
    loginTwoFactorAccount.value = loginAccount.value
...
function createAgent() {
  ...
  agentRows.value.push({ id: `A-${1000 + next}`, uid: `AG-${10000 + next}`, account, displayName: newAgentName.value.trim() || account, referralCode: `AGT-${newAgentCurrency.value}-${String(next).padStart(4, '0')}`, phone: newAgentPhone.value.trim(), email: newAgentEmail.value.trim(), contactMethod: '尚未設定', twoFactor: '未啟用', twoFactorBoundAt: twoFactorGlobalEnabled.value ? '待首次登入綁定' : '不要求綁定', twoFactorRequired: twoFactorGlobalEnabled.value, level, path: basePath, currency: newAgentCurrency.value, point: savedPoint, children: 0, walletBalance: 0, status: '啟用', planId: selected.id, model: selected.model, cycle: selected.cycle })
...
function toggleAgentTwoFactor(value: boolean) {
...
function prepareTwoFactorReset() {
  if (!selectedTwoFactorAgent.value) return
  twoFactorAdminStep.value = 'reset'
}
...
function completeTwoFactorReset() {
  ...
  forceTwoFactorRebind.value[selectedTwoFactorAgent.value.account] = true
...
function completeLoginTwoFactorBind() {
  ...
  delete forceTwoFactorRebind.value[loginTwoFactorAccount.value]
  showLoginTwoFactorSetup.value = false
  loggedIn.value = true
  logs.value.unshift({ time: '2026-09-02 11:00:00', type: '完成 Google Auth 綁定', actor: loginTwoFactorAccount.value, detail: `${loginTwoFactorAccount.value} 完成重設後的 Google Auth 綁定`, ip: '10.20.8.15' })
  showNotice('success', 'Google Auth 綁定完成', '驗證器已綁定，現在可以進入代理後台。')
}
    ...
    <div>
          ...
          <main class="content-area">
            ...
            <section v-else-if="activeKey === 'agents'">
              ...
              <template v-else-if="selectedAgent">
                <div class="detail-page-head"><div><button class="back-link" @click="showAgentDetail = false">← 返回代理管理</button><h1>代理詳情 · {{ selectedAgent.account }}</h1><p class="muted">查看代理帳號、代理線、傭金錢包與安全設定。</p></div><NTag type="info" round>{{ selectedAgent.level }}</NTag></div>
                <div class="detail-tabs"><button :class="{ active: detailTab === 'basic' }" @click="detailTab = 'basic'">基本資料</button><button :class="{ active: detailTab === 'wallet' }" @click="detailTab = 'wallet'">即時資料</button><button :class="{ active: detailTab === 'auth' }" @click="detailTab = 'auth'">Google Auth</button><button :class="{ active: detailTab === 'relationship' }" @click="detailTab = 'relationship'">代理關係</button><button :class="{ active: detailTab === 'commission' }" @click="detailTab = 'commission'">傭金設定</button><button :class="{ active: detailTab === 'logs' }" @click="detailTab = 'logs'">操作紀錄</button></div>
            ...
            <section v-else-if="activeKey === 'players'">
              ...
              <template v-else-if="selectedPlayer">
                <div class="detail-page-head"><div><button class="back-link" @click="showPlayerDetail = false">← 返回玩家管理</button><h1>玩家詳情 · {{ selectedPlayer.account }}</h1><p class="muted">查看玩家基本資料、VIP、資金、遊戲與代理關係。</p></div><NTag :type="playerStatusType(selectedPlayer.status)" round>{{ selectedPlayer.status }}</NTag></div>
                <div class="detail-tabs"><button :class="{ active: playerDetailTab === 'basic' }" @click="playerDetailTab = 'basic'">基本資料</button><button :class="{ active: playerDetailTab === 'wallet' }" @click="playerDetailTab = 'wallet'">即時資料</button><button :class="{ active: playerDetailTab === 'vip' }" @click="playerDetailTab = 'vip'">VIP 資訊</button><button :class="{ active: playerDetailTab === 'promotion' }" @click="playerDetailTab = 'promotion'">優惠紀錄</button><button :class="{ active: playerDetailTab === 'asset' }" @click="playerDetailTab = 'asset'">資金帳變</button><button :class="{ active: playerDetailTab === 'game' }" @click="playerDetailTab = 'game'">遊戲紀錄</button><button :class="{ active: playerDetailTab === 'agent' }" @click="playerDetailTab = 'agent'">代理關係</button></div>
                ...
                <div v-else class="agent-detail-grid detail-page-card"><div class="full"><span>完整樹狀路徑</span><strong>{{ selectedPlayer.path }}</strong></div><div><span>玩家層級</span><strong>{{ selectedPlayer.agentLevel }}</strong></div><div><span>所屬幣別</span><strong>{{ selectedPlayer.currency }}</strong></div><div class="full"><span>代理後台權限</span><p class="modal-help">總代理與一般代理可以查看所屬下級；轉線、狀態管理與編輯資料僅由運營商執行。</p></div></div>
              </template>
            </section>
      ...
      <NModal v-model:show="showLoginTwoFactorSetup" preset="card" title="首次登入：綁定 Google Auth" class="modal-card auth-admin-modal">
        <div class="auth-admin-panel">
          ...
          <p class="modal-help">運營商已重設此帳號的 2FA。請先完成 Google Auth 綁定，完成後才能進入代理後台。</p>
          <div class="auth-qr-layout"><div class="auth-qr" aria-label="Google Auth QR Code 示意"><span>QR</span></div><div><strong>使用 Google Authenticator 掃描</strong><ol><li>開啟 Google Authenticator。</li><li>掃描 QR Code，或手動輸入密鑰。</li><li>確認驗證碼後完成綁定。</li></ol><p class="auth-secret">手動密鑰：JBSW Y3DP EHPK 3PXP</p></div></div>
          <p class="modal-help">正式版應由後端驗證一次性密鑰，並在完成綁定前禁止進入後台。</p>
        </div>
        <template #footer><NSpace justify="end"><NButton type="primary" @click="completeLoginTwoFactorBind">我已完成綁定，進入後台</NButton></NSpace></template>
      </NModal>
      ...
      <NModal v-model:show="showTwoFactorAdmin" preset="card" title="Google Auth 管理" class="modal-card auth-admin-modal">
        <template v-if="selectedTwoFactorAgent">
          <div v-if="twoFactorAdminStep === 'overview'" class="auth-admin-panel">
            ...
            <NSpace><NButton v-if="selectedTwoFactorAgent.twoFactor === '已啟用'" secondary @click="viewTwoFactorQr">查看目前 QR Code</NButton><NButton type="warning" @click="prepareTwoFactorReset">重設並產生重新綁定 QR Code</NButton></NSpace>
          </div>
          <div v-else class="auth-admin-panel">
            <div class="auth-admin-heading"><div><NButton text size="small" @click="twoFactorAdminStep = 'overview'">← 返回 Google Auth 管理</NButton><span>{{ twoFactorAdminStep === 'reset' ? '重新綁定帳號' : '查看 Google Auth QR Code' }}</span><strong>{{ selectedTwoFactorAgent.account }}</strong></div><NTag :type="twoFactorAdminStep === 'reset' ? 'warning' : 'success'" round>{{ twoFactorAdminStep === 'reset' ? '待重新綁定' : '目前已綁定' }}</NTag></div>
            ...
            <p class="modal-help">{{ twoFactorAdminStep === 'reset' ? '此為原型示意。完成重設後，舊驗證器立即失效；代理下次登入需重新綁定。' : '此為原型示意。正式版 QR Code 與密鑰只顯示一次，查看行為會寫入操作日誌。' }}</p>
          </div>
        </template>
        <template #footer><NSpace justify="end"><NButton @click="showTwoFactorAdmin = false">取消</NButton><NButton v-if="twoFactorAdminStep === 'reset'" type="primary" @click="completeTwoFactorReset">完成重設</NButton></NSpace></template>
      </NModal>
      <NModal v-model:show="showPlayerDetailModal" preset="card" :title="selectedPlayer ? `玩家詳情 · ${selectedPlayer.account}` : '玩家詳情'" class="modal-card">
        <template v-if="selectedPlayer">
          ...
          <div v-else-if="playerDetailTab === 'wallet'" class="agent-detail-grid"><div><span>現金錢包</span><strong>{{ selectedPlayer.currency }} 12,800</strong></div><div><span>活動錢包</span><strong>{{ selectedPlayer.currency }} 3,260</strong></div><div><span>遊戲錢包</span><strong>{{ selectedPlayer.currency }} 1,840</strong></div><div><span>安全錢包</span><strong>{{ selectedPlayer.currency }} 0</strong></div><div class="full"><span>帳號權限</span><p class="modal-help">禁言：正常；優惠：開啟；儲值：開啟；遊戲：開啟。代理角色只能查看，不能修改權限。</p></div></div>
          <div v-else-if="playerDetailTab === 'vip'" class="agent-detail-grid"><div><span>VIP 等級</span><strong>{{ selectedPlayer.vipLevel }}</strong></div><div><span>本月 VIP 累計</span><strong>儲值 {{ selectedPlayer.deposit.toLocaleString() }}／投注 {{ selectedPlayer.bet.toLocaleString() }}</strong></div><div class="full"><span>VIP 資訊</span><p class="modal-help">VIP 升降級與獎勵依運營後台規則執行；代理後台僅提供查詢。</p></div></div>
          <div v-else-if="playerDetailTab === 'promotion'" class="detail-list"><div class="detail-list-row"><span>優惠紀錄</span><strong>本期已領取 2 筆，待領取 1 筆</strong></div><div class="detail-list-row"><span>目前優惠狀態</span><NTag type="success" round>可查看</NTag></div><p class="modal-help">優惠領取與派發由運營後台管理；代理只能查看下級玩家的優惠狀況。</p></div>
          <div v-else-if="playerDetailTab === 'audit'" class="detail-list"><div v-for="item in [{type:'登入',time:'2026-08-31 10:22:04'},{type:'VIP 等級更新',time:'2026-08-30 00:00:12'},{type:'領取優惠',time:'2026-08-29 13:42:10'}]" :key="item.time" class="detail-list-row"><span>{{ item.type }}</span><strong>{{ item.time }}</strong></div><p class="modal-help">此頁僅呈現該玩家的操作稽核摘要。</p></div>
          <div v-else-if="playerDetailTab === 'asset'" class="detail-list"><div class="detail-list-row"><span>最近一筆資金帳變</span><strong>{{ selectedPlayer.currency }} +500 儲值成功</strong></div><div class="detail-list-row"><span>本期資金異動筆數</span><strong>18 筆</strong></div><p class="modal-help">資金帳變僅供查看，不可在代理後台進行轉帳、加扣款或撤銷。</p></div>
          <div v-else-if="playerDetailTab === 'game'" class="detail-list"><div class="detail-list-row"><span>最近投注</span><strong>{{ selectedPlayer.currency }} {{ selectedPlayer.bet.toLocaleString() }}</strong></div><div class="detail-list-row"><span>最近遊戲</span><strong>Sport · Live Casino</strong></div><div class="detail-list-row"><span>RTP</span><strong :class="selectedPlayer.rtp < 100 ? 'positive' : 'negative'">{{ selectedPlayer.rtp }}%</strong></div></div>
          <div v-else-if="playerDetailTab === 'transfer'" class="detail-list"><div class="detail-list-row"><span>目前代理線</span><strong>{{ selectedPlayer.path }}</strong></div><div class="detail-list-row"><span>最近轉線</span><strong>尚無新的轉線紀錄</strong></div><p class="modal-help">玩家轉線由運營商操作；歷史訂單不回溯重算。</p></div>
          <div v-else-if="playerDetailTab === 'invite'" class="detail-list"><div class="detail-list-row"><span>註冊使用推廣碼</span><strong class="code-text">{{ selectedPlayer.referralCode || '未記錄' }}</strong></div><div class="detail-list-row"><span>成功邀請人數</span><strong>0 人</strong></div></div>
          <div v-else-if="playerDetailTab === 'agent'" class="agent-detail-grid"><div class="full"><span>完整樹狀路徑</span><strong>{{ selectedPlayer.path }}</strong></div><div><span>玩家層級</span><strong>{{ selectedPlayer.agentLevel }}</strong></div><div><span>所屬幣別</span><strong>{{ selectedPlayer.currency }}</strong></div><div class="full"><span>代理後台權限</span><p class="modal-help">總代理與一般代理可以查看所屬下級；轉線、狀態管理與編輯資料僅由運營商執行。</p></div></div>
        </template>
