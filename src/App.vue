...
function createAgent() {
  ...
  agentRows.value.push({ id: `A-${1000 + next}`, uid: `AG-${10000 + next}`, account, displayName: newAgentName.value.trim() || account, referralCode: `AGT-${newAgentCurrency.value}-${String(next).padStart(4, '0')}`, phone: newAgentPhone.value.trim(), email: newAgentEmail.value.trim(), contactMethod: '尚未設定', twoFactor: '未啟用', twoFactorBoundAt: twoFactorGlobalEnabled.value ? '待首次登入綁定' : '不要求綁定', level, path: basePath, currency: newAgentCurrency.value, point: savedPoint, children: 0, walletBalance: 0, status: '啟用', planId: selected.id, model: selected.model, cycle: selected.cycle })
