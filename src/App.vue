...
const agentWithdrawalRecords = computed<CommissionWithdrawalRecord[]>(() => {
  ...
  return [
    { id: 'WD-20260902-001', applyAt: '2026-09-02 11:26:08', settlePeriod: '2026-W35', amount: 12000, fee: 0, netAmount: 12000, status: '審核中', bank: '台新銀行 · ****9012', reviewAt: '-', reviewer: '待分派', remark: '本期傭金提領申請' },
    { id: 'WD-20260825-004', applyAt: '2026-08-25 09:14:32', settlePeriod: '2026-W34', amount: 8600, fee: 0, netAmount: 8600, status: '已撥款', bank: '台新銀行 · ****9012', reviewAt: '2026-08-25 14:08:21', reviewer: 'operator_demo', remark: `${currency} 傭金已匯入` },
    { id: 'WD-20260811-002', applyAt: '2026-08-11 16:42:10', settlePeriod: '2026-W32', amount: 15400, fee: 120, netAmount: 15280, status: '已撥款', bank: '台新銀行 · ****9012', reviewAt: '2026-08-12 10:02:44', reviewer: 'operator_demo', remark: '扣除跨行處理費' },
    { id: 'WD-20260728-006', applyAt: '2026-07-28 13:08:55', settlePeriod: '2026-W30', amount: 4200, fee: 0, netAmount: 4200, status: '已駁回', bank: '台新銀行 · ****9012', reviewAt: '2026-07-29 09:18:06', reviewer: 'operator_demo', remark: '收款帳戶驗證未完成' },
  ]
})
    ...
    <div>
          ...
          <main class="content-area">
            ...
            <section v-else-if="activeKey === 'agents'">
              ...
              <template v-else-if="selectedAgent">
                  ...
                  <div class="detail-tabs"><button :class="{ active: detailTab === 'wallet' }" @click="detailTab = 'wallet'">即時資料</button><button :class="{ active: detailTab === 'auth' }" @click="openTwoFactorAdminPage(selectedAgent)">Google Auth</button><button :class="{ active: detailTab === 'relationship' }" @click="detailTab = 'relationship'">代理資訊</button><button :class="{ active: detailTab === 'commission' }" @click="detailTab = 'commission'">傭金設定</button><button :class="{ active: detailTab === 'withdrawals' }" @click="detailTab = 'withdrawals'">傭金領取紀錄</button><button :class="{ active: detailTab === 'logs' }" @click="detailTab = 'logs'">操作紀錄</button></div>
                ...
                <div v-else-if="detailTab === 'withdrawals'" class="agent-withdrawal-panel detail-page-card"><div class="table-section-label"><strong>傭金領取紀錄</strong><span>查看此代理歷次傭金提領申請、審核與撥款狀態</span></div><NCard class="filter-card filter-card-emphasis agent-withdrawal-filter"><div class="filter-row filter-row-advanced"><div class="filter-field-group filter-date-group"><label>申請時間</label><NDatePicker v-model:value="agentWithdrawalDateRange" type="daterange" clearable format="yyyy-MM-dd" start-placeholder="申請起日" end-placeholder="申請迄日" placeholder="選擇時間區段" class="filter-date" /></div><div class="filter-field-group"><label>領取狀態</label><NSelect v-model:value="agentWithdrawalStatus" clearable placeholder="全部狀態" :options="[{label:'待審核',value:'待審核'},{label:'審核中',value:'審核中'},{label:'已撥款',value:'已撥款'},{label:'已駁回',value:'已駁回'}]" class="filter-select" /></div><div class="filter-field-group filter-account-group"><label>關鍵字／單號</label><NInput v-model:value="agentWithdrawalKeyword" clearable placeholder="搜尋提領單號、銀行或備註" class="filter-field filter-log-search"><template #prefix><NIcon><SearchOutline /></NIcon></template></NInput></div></div></NCard><NCard :bordered="false" class="table-card withdrawal-record-card"><div class="withdrawal-record-head"><span>申請時間</span><span>提領單號</span><span>結算週期</span><span>申請金額</span><span>實際入帳</span><span>收款帳戶</span><span>狀態</span><span>審核資訊</span></div><div v-for="record in filteredAgentWithdrawalRecords" :key="record.id" class="withdrawal-record-row"><time>{{ record.applyAt }}</time><code>{{ record.id }}</code><span>{{ record.settlePeriod }}</span><strong>{{ selectedAgent.currency }} {{ record.amount.toLocaleString() }}</strong><strong>{{ selectedAgent.currency }} {{ record.netAmount.toLocaleString() }}</strong><span>{{ record.bank }}</span><NTag size="small" round :type="record.status === '已撥款' ? 'success' : record.status === '已駁回' ? 'error' : 'warning'">{{ record.status }}</NTag><div><strong>{{ record.reviewAt }}</strong><small>{{ record.reviewer }} · {{ record.remark }}</small></div></div><p v-if="!filteredAgentWithdrawalRecords.length" class="modal-help">目前沒有符合條件的傭金領取紀錄。</p></NCard></div>
            ...
            <section v-else-if="activeKey === 'players'">
              ...
              <template v-else-if="selectedPlayer">
                ...
                <div v-else-if="playerDetailTab === 'promotion'" class="detail-list detail-page-card"><div class="detail-list-row"><span>優惠紀錄</span><strong>本期已領取 2 筆，待領取 1 筆</strong></div><div class="detail-list-row"><span>最近活動</span><strong>VIP3 升級禮 · 已完成</strong></div><div class="detail-list-row"><span>待領取優惠</span><strong>{{ selectedPlayer.currency }} 800 · 首存回饋</strong></div><div class="detail-list-row"><span>最後領取時間</span><strong>2026-08-29 13:42:10</strong></div><div class="detail-list-row"><span>流水要求</span><strong>8 倍（目前完成 6.4 倍）</strong></div><p class="modal-help">優惠領取與派發由運營後台管理；代理只能查看。</p></div>
                <div v-else-if="playerDetailTab === 'asset'" class="detail-list detail-page-card"><div class="detail-list-row"><span>最近一筆資金帳變</span><strong>{{ selectedPlayer.currency }} +500 儲值成功</strong></div><div class="detail-list-row"><span>本期資金異動筆數</span><strong>18 筆</strong></div><div class="detail-list-row"><span>本月儲值總額</span><strong>{{ selectedPlayer.currency }} {{ selectedPlayer.deposit.toLocaleString() }}</strong></div><div class="detail-list-row"><span>本月提領總額</span><strong>{{ selectedPlayer.currency }} 4,200</strong></div><div class="detail-list-row"><span>最後異動時間</span><strong>2026-08-31 10:22:04</strong></div></div>
                 <div v-else-if="playerDetailTab === 'game'" class="detail-list detail-page-card"><div class="detail-list-row"><span>最近投注</span><strong>{{ selectedPlayer.currency }} {{ selectedPlayer.bet.toLocaleString() }}</strong></div><div class="detail-list-row"><span>本期有效投注</span><strong>{{ selectedPlayer.currency }} 86,420</strong></div><div class="detail-list-row"><span>最近遊戲</span><strong>Sport · Live Casino</strong></div><div class="detail-list-row"><span>熱門遊戲</span><strong>足球盤口／百家樂</strong></div><div class="detail-list-row"><span>RTP</span><strong :class="selectedPlayer.rtp < 100 ? 'positive' : 'negative'">{{ selectedPlayer.rtp }}%</strong></div><div class="detail-list-row"><span>最近遊戲時間</span><strong>2026-08-31 10:18:42</strong></div></div>
                 <div v-else-if="playerDetailTab === 'audit'" class="detail-list detail-page-card"><div class="detail-list-row"><span>最近操作</span><strong>查看玩家詳情</strong></div><div class="detail-list-row"><span>操作角色</span><strong>{{ identity.label }}</strong></div><div class="detail-list-row"><span>最近登入 IP</span><strong>{{ selectedPlayer.lastLoginIp }}</strong></div><div class="detail-list-row"><span>最近異動</span><strong>VIP2 → VIP3</strong></div><div class="detail-list-row"><span>稽核筆數</span><strong>12 筆</strong></div><p class="modal-help">玩家相關查詢與操作依角色權限記錄；完整稽核資料由平台操作日誌保存。</p></div>
                 <div v-else-if="playerDetailTab === 'transfer'" class="detail-list detail-page-card"><div class="detail-list-row"><span>目前代理線</span><strong>{{ displayAgentPath(selectedPlayer.path) }}</strong></div><div class="detail-list-row"><span>原代理線</span><strong>平台 > agent_taipei > north_team</strong></div><div class="detail-list-row"><span>最近轉線</span><strong>2026-08-01 00:00:00</strong></div><div class="detail-list-row"><span>轉線生效時間</span><strong>2026-08-01 00:00:00</strong></div><div class="detail-list-row"><span>執行人</span><strong>operator_demo</strong></div><p class="modal-help">玩家轉線由運營商操作；歷史訂單不回溯重算。</p></div>
      ...
      <NModal v-model:show="showAgentDetailModal" preset="card" :title="selectedAgent ? `代理詳情 · ${selectedAgent.account}` : '代理詳情'" class="modal-card">
        <template v-if="selectedAgent">
           <div class="detail-tabs"><button :class="{ active: detailTab === 'basic' }" @click="detailTab = 'basic'">基本資料</button><button :class="{ active: detailTab === 'wallet' }" @click="detailTab = 'wallet'">即時資料</button><button :class="{ active: detailTab === 'relationship' }" @click="detailTab = 'relationship'">代理資訊</button><button :class="{ active: detailTab === 'commission' }" @click="detailTab = 'commission'">傭金設定</button><button :class="{ active: detailTab === 'withdrawals' }" @click="detailTab = 'withdrawals'">傭金領取紀錄</button><button :class="{ active: detailTab === 'logs' }" @click="detailTab = 'logs'">操作紀錄</button></div>
           ...
           <div v-else-if="detailTab === 'withdrawals'" class="agent-withdrawal-panel"><div class="table-section-label"><strong>傭金領取紀錄</strong><span>歷次提領申請與審核狀態</span></div><div class="filter-row filter-row-advanced agent-withdrawal-filter"><div class="filter-field-group filter-date-group"><label>申請時間</label><NDatePicker v-model:value="agentWithdrawalDateRange" type="daterange" clearable format="yyyy-MM-dd" placeholder="選擇時間區段" class="filter-date" /></div><div class="filter-field-group"><label>領取狀態</label><NSelect v-model:value="agentWithdrawalStatus" clearable placeholder="全部狀態" :options="[{label:'待審核',value:'待審核'},{label:'審核中',value:'審核中'},{label:'已撥款',value:'已撥款'},{label:'已駁回',value:'已駁回'}]" class="filter-select" /></div><div class="filter-field-group filter-account-group"><label>關鍵字／單號</label><NInput v-model:value="agentWithdrawalKeyword" clearable placeholder="搜尋單號、銀行或備註" class="filter-field" /></div></div><div class="withdrawal-record-list"><div v-for="record in filteredAgentWithdrawalRecords" :key="record.id" class="withdrawal-record-item"><div><strong>{{ record.id }}</strong><small>{{ record.applyAt }} · {{ record.settlePeriod }}</small></div><div><strong>{{ selectedAgent.currency }} {{ record.amount.toLocaleString() }}</strong><small>實際入帳 {{ selectedAgent.currency }} {{ record.netAmount.toLocaleString() }}</small></div><NTag size="small" round :type="record.status === '已撥款' ? 'success' : record.status === '已駁回' ? 'error' : 'warning'">{{ record.status }}</NTag></div><p v-if="!filteredAgentWithdrawalRecords.length" class="modal-help">目前沒有符合條件的傭金領取紀錄。</p></div></div>
